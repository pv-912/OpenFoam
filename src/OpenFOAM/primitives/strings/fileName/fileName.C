...
#ifdef MSWIN
    // Windows absolute paths start with "C:/" or "C:\"
    if (size() > 3 && isalpha((*this)[0]) && (*this)[1] == ':' && ((*this)[2] == '/' || (*this)[2] == '\\'))
        return true;

    // Windows network paths start with "\\"
...
bool Foam::fileName::clean()
{
    ...
    string::size_type top = find_first_of("/\\");
    ...
    )
    {
        ...
        if (prev == '/')
        {
            ...
            if (c == '.')
            {
                ...
                // It is '/..' or '/../'
                if (c1 == '.' && (src+1 >= maxLen || operator[](src+1) == '/' || operator[](src+1) == '\\'))
                {
                    ...
                    (
                        nChar > 2
                     && ((parent = rfind('/', nChar-2)) != npos || (parent = rfind('\\', nChar-2)) != npos)
                     && parent >= top
                    )
                    {
    ...
    if (nChar > 1 && (operator[](nChar-1) == '/' || operator[](nChar-1) == '\\'))
    {
        nChar--;
    }
...
Foam::fileName Foam::fileName::path() const
{
    ...
    {
        // do not discard trailing slash in "C:\"
        return substr(0, (*this)[i-1] == ':' ? i + 1 : i);
    }
    else
    {
        return "/";
...
Foam::fileName Foam::fileName::lessExt() const
{
    ...
    if (i == npos || i == 0 || operator[](i) == '/' || operator[](i) == '\\')
    {
        return *this;
    }
    else
